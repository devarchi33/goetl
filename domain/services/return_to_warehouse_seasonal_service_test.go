...
func rtwSeasonalSyncedShouldBeTrueOrFalse(shipmentLocationCode, waybillNo string, shouldBeTrue bool) {
...
func TestReturnToWarehouseSeasonalETL(t *testing.T) {
	Convey("测试ReturnToWarehouseSeasonalETL的Run方法", t, func() {
		etl := ReturnToWarehouseSeasonalETL{}.New()
		err := etl.Run(context.Background())
		So(err, ShouldBeNil)
		waybillNo := "S20190819001"
		...
		Convey("运单号为"+waybillNo+"的退仓出库单应该在CSL有记录", func() {
			...
			for _, recvSupp := range recvSuppList {
				...
				So(recvSupp.ShippingTypeCode, ShouldEqual, cslConst.TypRTWSeasonal)
			...
			rtwSeasonalSyncedShouldBeTrueOrFalse("CEGP", waybillNo, true)
		})

		waybillNo = "S20190814001"
